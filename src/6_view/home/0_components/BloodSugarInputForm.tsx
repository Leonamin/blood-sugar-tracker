...
export default function BloodSugarInputForm({
  ...
  const level = BloodSugarCategoryUtils.getGlucoseLevel(
    bloodSugarCategory,
    parseSafeValue(value)
  );
  const step =
    parseSafeValue(value) < minValue
      ? 0
      : GlucoseLevelUtils.getIndicatorStep(level);
    ...
    <div className="relative flex flex-col items-center">
      ...
      <div className="absolute bottom-0 flex items-center gap-1">
        <input
          ...
          maxLength={5}
          className={twMerge(
            "w-16 bg-transparent text-center outline-none",
            "text-h3b text-right color-text-primary",
            "caret-indigo-500 dark:caret-indigo-300"
          )}
        />
        <span className="text-body-1m color-text-tertiary">{unit}</span>
      </div>
    </div>
  );
}
